...
module.exports.udapMetadataHandler = async () => {
	const serverCert = fs.readFileSync(process.env.SERVER_PUBLIC_CERT, {encoding: 'utf-8'})
	const serverKey = fs.readFileSync(process.env.SERVER_PRIVATE_KEY, {encoding: 'utf-8'})
	...
	if(serverCertSAN == serverCertSAN) {// TODO: Put this back in. process.env.FHIR_BASE){
	    ...
	    "registration_endpoint": process.env.AUTHZ_BASE + '/register',
			"registration_endpoint_jwt_signing_alg_values_supported": ["RS256"],
	    "authorization_endpoint" : process.env.AUTHZ_ISSUER + '/v1/authorize',
	    "token_endpoint":  process.env.AUTHZ_ISSUER + '/v1/token',
			"token_endpoint_auth_signing_alg_values_supported":["RS256"],
			"token_endpoint_auth_methods_supported": ["private_key_jwt"],
			"signed_metadata": getSignedEndpointsJWT(serverCert, serverKey)
		}
	}
	else {
...
function getSignedEndpointsJWT(serverCert, serverKey) {
	...
	const jwt = njwt.create(claims, stripHeadTail(serverKey))
		.setIssuedAt(now)
