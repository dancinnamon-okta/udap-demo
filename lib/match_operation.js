 ...
 */
 ...
 if(matchParameters.resource.identifier) {
  ...
  matchedPatients.forEach(function(value, key) {
    ...
    if((!matchParameters.onlyCertainMatches || value.score >= 0.8) && (outputEntries.length < matchParameters.count)){
      outputEntries.push({
        ...
        "search": {
          "extension": [{
            ...
            "valueCode": value.score >= 0.8 ? "certain" : "possible"
          }],
          "mode": "match",
          "score": matchScore
        }
...
function validateMatchParameters(resource, minValidationLevel) {
  ...
  if(resource.identifier) {
    for(var i=0; i<resource.identifier.length; i++) {
      if(resource.identifier[i].type && ["DL", "STID"].includes(resource.identifier[i].type.coding.code) && resource.identifier[i].value) {
        idDLSTIDScore += 1
      }
      else if(resource.identifier[i].type && resource.identifier[i].type.coding.code == "PPN" && resource.identifier[i].value) {
        idPPNScore += 1
      }
