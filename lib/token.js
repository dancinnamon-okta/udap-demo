	...
	if(oktaFormData) {
		try {
			var oktaResponse = await axios.request({
				'url': tokenEndpoint,
				'method': 'POST',
				'headers': {'Content-Type': 'application/x-www-form-urlencoded', 'Host': process.env.AUTHZ_ISSUER_HOSTNAME},
				'data': oktaFormData
			})
...
function get_okta_token_request(requestBody, requestHeaders, jwtBody) {
	...
	formBody += '&client_id=' + jwtBody.sub
...
function validateUdapProperties(jwtAntBody)
{
    ...
    console.log('expected aud: ' + process.env.AUTHZ_ISSUER + '/v1/token')
    if (!jwtAntBody.hasOwnProperty('aud') || jwtAntBody.aud  == '' || jwtAntBody.aud != process.env.AUTHZ_ISSUER + '/v1/token')
    {
