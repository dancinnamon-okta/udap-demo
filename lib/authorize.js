	...
	if(idpUri && scopes && scopes.split(" ").includes("udap")) {
		...
		if(idpValidationResult.valid === 'true') {
			...
			if(!oktaIDPId) {
				...
				const oidcMetadata = await readOIDCConfiguration(idpValidationResult.validatedMetadataBody.authorization_endpoint)
...
//This function will:
//take the /authorize url given to us in /.well-known/udap, and then attempt to find the OIDC .well-known endpoint so we can get the /keys endpoint and /userinfo.
//TODO: Discuss with the FAST team to see what better way there is to handle something like this.
async function readOIDCConfiguration(authorizeUrl) {

	//Huge hack to calculate the well-known endpoint.
	const oidcWellKnownEndpoint = authorizeUrl.replace('/v1/authorize', '/.well-known/openid-configuration')
